...
$(document).ready(function () {
  ...
  function drawVisualization() {
    ...
    const purpleAir = {
      highest: 0,
      lowest: 100,
      highID: "purple-air-highest",
      lowID: "purple-air-lowest",
    };

    var purpleAirChart = new google.visualization.LineChart(
      $("#purple-air-chart")[0],
    );
    var purpleAirData = google.visualization.arrayToDataTable([
      ["Time", "PurpleAir Temperature"],
      [getTime(), 0],
    ]);
    ...
    setInterval(doAJAXAirPoll, 1000);

    // TODO 7: WebSocket Polling
    // Polling for AJAX Temperature
    function doAJAXTempPoll() {
      $.ajax({
        url: "http://localhost:8080/",
        method: "GET",
        dataType: "json",
    ...
    setInterval(doAJAXTempPoll, 1000);
    ...
    // TODO 9: AJAX Polling
    function doPurpleAirAJAXPoll() {
      $.ajax({
        url: "https://api.purpleair.com/v1/sensors/300625?fields=temperature",
        method: "GET",
        headers: {
          "X-API-KEY": "place holder for api key",
        },
        dataType: "json",
        success: function (result) {
          addDataPoint(result, purpleAirData, purpleAirChart);
          updateRecords(purpleAir, result.value);
        },
    ...
    setInterval(doPurpleAirAJAXPoll, 5000);
