...
(function (window) {
  ...
  function blurFilterOn(displayObject, blurX = 5, blurY = 5, quality = 1) {
    ...
    displayObject.filters = displayObject.filters
      ? displayObject.filters.concat(blurFilter)
      : [blurFilter];
    displayObject.cache(
      -displayObject.radius,
      -displayObject.radius,
      displayObject.width,
  ...
  const draw = {
    ...
    setDimensionsOn: function (shape, dimensions) {
      ...
      if (shape.dimensions) {
        ...
        const xStartPoint = [
            getStartPointX(shape),
            getStartPointX(dimensions),
          ].sort(sortNumbersAscending)[0],
          xEndPoint = [getEndPointX(shape), getEndPointX(dimensions)].sort(
            sortNumbersAscending
          )[1],
          yStartPoint = [
            getStartPointY(shape),
            getStartPointY(dimensions),
          ].sort(sortNumbersAscending)[0],
          yEndPoint = [getEndPointY(shape), getEndPointY(dimensions)].sort(
            sortNumbersAscending
          )[1];
    ...
    line: function (fromX, fromY, toX, toY, strokeColor, strokeStyle, onShape) {
      const dimensions = buildDimensions(TYPE_LINEAR, toX, toY, fromX, fromY),
    ...
    randomCircleInArea: function (
      ...
      props = randomRadialProps
        ? randomRadialProps
        : draw.randomRadialProps(area);
    ...
    fps: function (color = "#FFF") {
      const textfield = new createjs.Text("-- fps", "bold 30px Arial", color),
